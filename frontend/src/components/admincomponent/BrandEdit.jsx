...
export default function BrandEdit({editdeta}) {
...
function submitHandler(e) {
  ...
  .then((res) => {
    if (res.data.success) {
      ...
      router.push("/admin/brand"); // ✅ corrected
    }
  })
  .catch((error) => {
...
/>
        ...
        <div className="px-6 py-4 text-gray-800">
            <img className="w-10 h-10 rounded-md" src={process.env.NEXT_PUBLIC_BRAND_IMAGE_URL+editdeta.image} alt={editdeta.name} />
          </div> 
      </div>
