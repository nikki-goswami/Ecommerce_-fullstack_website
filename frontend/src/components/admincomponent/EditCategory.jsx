...
//     payload.append("image",e.target.image.value)
   
//     axiosbaseURL.put(`category/update/${editdeta._id}`,payload).then(
//       (res)=>{
//         if(res.data.success){
...
function submitHandler(e) {
  ...
  axiosbaseURL.put(`category/update/${editdeta._id}`, payload,
    {
      headers: {
  ...
  .then((res) => {
    if (res.data.success) {
      ...
      router.push("/admin/category"); // ✅ corrected
    }
  })
  .catch((error) => {
    notify(error?.response?.data?.message, false);
...
/>
        ...
        <div className="px-6 py-4 text-gray-800">
            <img className="w-10 h-10 rounded-md" src={process.env.NEXT_PUBLIC_CATEGORY_IMAGE_URL+editdeta.image} alt={editdeta.name} />
          </div> 
      </div>
